...
Output:
    ...
    '''
def answer(l,t):
    #checks to see if it is possible
    if sum(l) < t:
        return [-1,-1]
    #checks to see if the whole list is the answer
    if sum(l) == t:
        return [0,len(l)]
    i = 0
    temp = 0
    
    #loops through the list, l
    while i < len(l):
        temp = l[i] #sets the total to the # @ i in the list
        j = i + 1 
        #loops through adding the rest of the loop to temp 
        while j < len(l) :
            temp = temp + l[j]
            if temp == t: #checks for the correct ans
                return [i, j]
            if temp > t: #sees if the answer is too big
                temp = temp - l[i] #if so takes the first number off
                break # then resumes the first loop
            j = j + 1
        i = i + 1            
